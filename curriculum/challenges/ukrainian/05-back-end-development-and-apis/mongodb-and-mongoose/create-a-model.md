...
Gitpod is a real server, and in real servers, the interactions with the database happen in handler functions. Ці функції виконуються, коли трапляється якась подія (наприклад, хтось попадає на кінцеву точку вашого API). Ми будемо дотримуватись того самого підходу в цих вправах. Функція `done()` — це зворотний виклик, який повідомляє нас про те, що ми можемо продовжувати після завершення асинхронної операції, як-от як вставка, пошук, оновлення чи видалення. Вона відповідає конвенції Node і повинна називатися `done(null, data)` в разі удачі або `done(err)` в разі помилки.
