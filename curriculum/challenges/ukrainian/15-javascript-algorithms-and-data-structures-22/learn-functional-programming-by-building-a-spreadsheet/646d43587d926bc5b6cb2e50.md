...
Використайте `const`, щоб оголосити змінну `str2`.
...
Передайте `noHigh` як перший аргумент до `infixEval()`.
...
Передайте `infix` як другий аргумент до `infixEval()`.
